...
def evaluate(eq):
	...
	eqs = eq.split(' ');
	if len(eqs) == 1: #symbol case
		res = evaluate(wires[eqs[0]])
	elif len(eqs) == 2: #not case
		res = ~evaluate(eqs[1])
	elif 'AND' in eqs:
		res = evaluate(eqs[0]) & evaluate(eqs[2])
	elif 'OR' in eqs:
		res = evaluate(eqs[0]) | evaluate(eqs[2])
	elif 'LSHIFT' in eqs:
		res = evaluate(eqs[0]) << evaluate(eqs[2])
	...
	res = res & 65535  #mask the result to 16 bits
